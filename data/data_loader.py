        ...
        for start, end in date_ranges:
            for ticker in tickers:
                ...
                if not df.empty:
                    ...
                    if ticker == "^GSPC":
                        df = df[["Close_^GSPC"]].rename(
                            columns={"Close_^GSPC": "SP500"}
                        )
                    else:  # SI=F
                        df = df[
                            [
                    ...
                    print(
                        f"Fetched {ticker} shape: {df.shape}, \
columns: {df.columns.tolist()}"
                    )
                    all_data[ticker].append(df)
                else:
                    print(
                        f"Warning: No data fetched for {ticker} \
from {start} to {end}"
                    )
        ...
        for ticker in tickers:
            if all_data[ticker]:
                ...
                if ticker == "SI=F":
                    print(
                        f"Silver data shape: {df.shape}, \
non-NaN Close counts: {df['Close'].count()}"
                    )
                else:  # ^GSPC
                    print(
                        f"S&P 500 data shape: {df.shape}, \
non-NaN SP500 counts: {df['SP500'].count()}"
                    )
        ...
        if combined_data:
            ...
            for df in combined_data[1:]:
                # Left join to keep silver data
                full_df = full_df.join(df, how="left")
            print(f"Combined DataFrame columns: {full_df.columns.tolist()}")
            print(
                f"Combined DataFrame shape: {full_df.shape}, non-NaN \
SP500 counts: {full_df['SP500'].count() if 'SP500' in full_df.columns else 0}"
            )
    ...
    def _calculate_indicators(self, df):
        ...
        for col in expected_columns:
            if col in df.columns:
                ...
                print(f"Warning: Column {col} missing in \
DataFrame. Adding with NaN.")
                df[col] = np.nan
        ...
        if "SP500" in df.columns:
            ...
            df["SP500"] = df["SP500"].interpolate(
                method="time",
                limit_direction="both"
            )
            non_nan_after = df["SP500"].count()
            print(
                f"SP500 non-NaN counts before interpolation: \
{non_nan_before}, after: {non_nan_after}"
            )
            print(f"SP500 sample values:\n{df['SP500'].head()}")
            if non_nan_after < len(df) * 0.1:
                print(
                    f"Warning: SP500 has very few non-NaN values \
({non_nan_after}/{len(df)}) after interpolation."
                )
        else:
            print("Warning: SP500 column missing after \
processing. Adding with NaN.")
            df["SP500"] = np.nan
        ...
        print(
            f"Processed DataFrame shape: {df.shape}, \
non-NaN SP500 counts: \
{df['SP500'].count() if 'SP500' in df.columns else 0}"
        )
    ...
    def _needs_update(self):
        ...
        return True  # Remove this line and uncomment below after
        # confirming SP500 is included
        # if not os.path.exists(self.processed_data_path):
        #     return True
    ...
    def load_data(self):
        ...
        if self._needs_update():
            ...
            if "SP500" not in self.data.columns:
                print("Warning: SP500 column missing in \
loaded data. Adding with NaN.")
                self.data["SP500"] = np.nan
        print(f"Final DataFrame columns: {self.data.columns.tolist()}")
        print(
            f"Final DataFrame shape: {self.data.shape}, \
non-NaN SP500 counts: \
{self.data['SP500'].count() if 'SP500' in self.data.columns else 0}"
        )
