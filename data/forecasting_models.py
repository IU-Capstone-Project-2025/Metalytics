...
class ForecastModel(ABC):
    ...
    def fit(self, df: pd.DataFrame, params: Dict[str, Any]):
        """
        Creates dataset from `df` and fits the model with parameters to
        the dataset.
...
def compose_forecast_frame(
    ...
    Parameters:
        data (np.array): 2D-array of target observations.
        features (np.array): 1D-array of current forecast
        timeframe features.
    ...
    def predict(self, date_range: pd.DatetimeIndex) -> pd.Series:
        ...
        for date in date_range:
            ...
            x_ = compose_forecast_frame(
                history_df.to_numpy(), features.to_numpy(), self.lag
            )
    ...
    def predict(self, date_range: pd.DatetimeIndex) -> pd.Series:
        ...
        for idx, date in enumerate(date_range):
            ...
            if idx < len(date_range) - 1:
                ...
                # Set `Open` price
                prediction_df.loc[date_range[idx + 1], "Open"] = history_df[
                    "Close"
                ].iloc[-1]
...
class LSTMCloseFM(ForecastModel):
    def __init__(self, train_size=0.7):
        ...
        self.target_names = ["Close", "High", "Low", "Volume"]
        self.indicators = [
            "EMA20",
            "RSI14",
            "ATR14",
    ...
    def predict(self, date_range: pd.DatetimeIndex) -> pd.Series:
        ...
        prediction_df = self.build_forecast_data(pd.DataFrame(
            index=date_range
        ))
        ...
        for idx, date in enumerate(date_range):
            x_ = (
                history_df.iloc[-self.params["lag"]:]
                .to_numpy()
                .reshape(1, -1, self.params["lag"])
            )
            ...
            history_df["EMA20"] = ta.trend.EMAIndicator(
                history_df["Close"], window=20
            ).ema_indicator()
            history_df["RSI14"] = ta.momentum.RSIIndicator(
                history_df["Close"], window=14
            ).rsi()
...
class ClosePriceFM_Silver(ForecastModel):
    ...
    Attributes:
        model_ (BaseEstimator): regression model.
        feature_models (Dict[str, ForecastModel]): dictionary of
        auxiliary models and their names.
        df_ (pd.DataFrame): dataset built for training.
        last_close_price (float): price of the last observed close prices.
        lag (int): number of lagged features.
        indicators (List[str]): names of indicators.
    ...
    def predict(
        self, date_range: pd.DatetimeIndex, initial_price: float = None
    ) -> pd.Series:
