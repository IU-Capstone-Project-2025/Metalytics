...
class SLFM(ForecastModel):
    ...
    def build(self, df: pd.DataFrame) -> pd.DataFrame:
        prices_reindexed = df[self.feature_name].copy()

        # self.last_value = prices_reindexed.iloc[-1]

        # First difference
        # prices_reindexed = prices_reindexed.diff()
        # prices_reindexed.dropna(inplace=True)

        prices_reindexed = self.scaler_.fit_transform(pd.DataFrame(prices_reindexed))

        return prices_reindexed.astype(np.float32)
    ...
    def predict(self, date_range: pd.DatetimeIndex) -> pd.Series:
        ...
        # feature_diff_scaled = self.model_fit.predict(start=date_range[0], end=date_range[-1], dynamic=False)
        feature_diff_scaled = self.model_fit.forecast(steps=len(date_range))

        feature_diff = self.scaler_.inverse_transform(pd.DataFrame(feature_diff_scaled))

        return pd.Series(feature_diff.ravel(), dtype=np.float32, index=date_range)

        # value_predicted = self.last_value + np.cumsum(feature_diff)

        # return pd.Series(value_predicted.ravel(), dtype=np.float32, index=date_range)

    def dump(self, path: str) -> None:
        self.model_fit.save(f"{path}/{self.feature_name}_predictor.joblib")
        joblib.dump(self.scaler_, f"{path}/{self.feature_name}_scaler.pkl")

    def load(self, df: pd.DataFrame, path: str) -> None:
        self.model_fit = ARIMAResults.load(f"{path}/{self.feature_name}_predictor.joblib")
        self.scaler_ = joblib.load(f"{path}/{self.feature_name}_scaler.pkl")
        # self.last_value = df[self.feature_name].iloc[-1]
...
class VolumeFM(ForecastModel):
    ...
    def build(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Scaling of target
        self.scaler_.fit(df[['Volume']])
        df['Volume'] = self.scaler_.transform(df[['Volume']])
    ...
    def predict(self, date_range: pd.DatetimeIndex) -> pd.Series:
        ...
        volume_scaled = history_df.loc[date_range[0]:date_range[-1], 'Volume']
        volume_predicted = self.scaler_.inverse_transform(pd.DataFrame(volume_scaled))

        return pd.Series(volume_predicted.ravel(), dtype=np.float32, index=date_range)

    def dump(self, path: str) -> None:
        joblib.dump(self.model_, f"{path}/Volume_predictor.joblib")
        joblib.dump(self.scaler_, f"{path}/Volume_scaler.pkl")

    def load(self, df: pd.DataFrame, path: str):
        self.df_ = self.build(df)
        self.model_ = joblib.load(f"{path}/Volume_predictor.joblib")
        self.scaler_ = joblib.load(f"{path}/Volume_scaler.pkl")
...
class ClosePriceFM(ForecastModel):
    ...
    lag: int
    train_size: float
    features: List[str] = ['High', 'Low', 'Volume', 'Open']
    ...
    def build(self, df: pd.DataFrame):
        ...
        df = df[self.features + ['Close'] + self.indicators].copy()
        ...
        df.loc['EMA20'] = ta.trend.EMAIndicator(df['Close'], window=20).ema_indicator().iloc[-1]
        df.loc['RSI14'] = ta.momentum.RSIIndicator(df['Close'], window=14).rsi().iloc[-1]
        df.loc['ATR14'] = ta.volatility.AverageTrueRange(
            df['High'], df['Low'], df['Close'], window=14
        ).average_true_range().iloc[-1]

        macd = ta.trend.MACD(df['Close'], window_slow=26, window_fast=12, window_sign=9)
        df.loc['MACD'] = macd.macd().iloc[-1]
        df.loc['MACD_Signal'] = macd.macd_signal().iloc[-1]
        df.loc['MACD_Hist'] = macd.macd_diff().iloc[-1]

        df.dropna(inplace=True)

        # Normalization
        df[self.features + self.indicators] = MinMaxScaler().fit_transform(df[self.features + self.indicators])

        # Scaling target
        self.scaler_.fit(df[['Close']])
        df['Close'] = self.scaler_.transform(df[['Close']])
    ...
    def build_forecast_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Predict feature targets and normalize
        for feature, feature_model in self.feature_models.items():
            df.loc[:, feature] = feature_model.predict(df.index)
            print(feature)
            df.loc[:, feature] = MinMaxScaler().fit_transform(df[[feature]]).ravel().astype(np.float32)
    ...
    def predict(self, date_range: pd.DatetimeIndex) -> pd.Series:
        ...
        for idx, date in enumerate(date_range):
            ...
            history_close_price = pd.concat([history_df['Close'], pd.Series(y_, index=[date])]).astype(np.float32)
            ...
            # Indicators
            features.loc['EMA20'] = np.float32(
                ta.trend.EMAIndicator(history_close_price, window=20).ema_indicator().iloc[-1]
            )
            features.loc['RSI14'] = np.float32(
                ta.momentum.RSIIndicator(history_close_price, window=14).rsi().iloc[-1]
            )
            features.loc['ATR14'] = np.float32(ta.volatility.AverageTrueRange(
            ...
            features.loc['MACD'] = np.float32(macd.macd().iloc[-1])
            features.loc['MACD_Signal'] = np.float32(macd.macd_signal().iloc[-1])
            features.loc['MACD_Hist'] = np.float32(macd.macd_diff().iloc[-1])
            ...
            history_df = pd.concat([history_df, pd.DataFrame.from_records([features], index=[date])]).astype(np.float32)

        close_price_diff_scaled = history_df.loc[date_range[0]:date_range[-1], 'Close']
        close_price_diff = self.scaler_.inverse_transform(pd.DataFrame(close_price_diff_scaled))
        close_price_prediction = self.last_close_price + np.cumsum(close_price_diff)

        return pd.Series(close_price_prediction, dtype=np.float32, index=date_range)

    def dump(self, path: str) -> None:
        joblib.dump(self.model_, f"{path}/Close_predictor.joblib")
        joblib.dump(self.scaler_, f"{path}/Close_scaler.pkl")
    ...
    def load(self, df: pd.DataFrame, path: str):
        ...
        self.scaler_ = joblib.load(f"{path}/Close_scaler.pkl")
        for model in self.feature_models.values():
            model.load(df, path)
